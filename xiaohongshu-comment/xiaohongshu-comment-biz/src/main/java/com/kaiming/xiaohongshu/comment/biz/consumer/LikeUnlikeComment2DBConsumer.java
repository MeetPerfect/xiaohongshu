...
/**
 * ClassName: LikeUnlikeComment2DBConsumer
 * Package: com.kaiming.xiaohongshu.comment.biz.consumer
 * Description:
 *
 * @Auther gongkaiming
...
public class LikeUnlikeComment2DBConsumer {
                ...
                commentIdAndListMap.forEach((commentId, ops) -> {
                    ...
                    Map<Long, LikeUnlikeCommentMqDTO> userLastOp = ops.stream()
                            .collect(Collectors.toMap(
                                    ...
                                    (oldValue, newValue) -> oldValue.getCreateTime().isAfter(newValue.getCreateTime()) ? oldValue : newValue
                            ));
    ...
    private void executeBatchSQL(List<LikeUnlikeCommentMqDTO> values) {
        ...
        List<LikeUnlikeCommentMqDTO> likes  = values.stream()
                .filter(op -> Objects.equals(LikeUnlikeCommentTypeEnum.LIKE.getCode(), op.getType()))
                .toList();

        // 过滤出取消点赞操作
        List<LikeUnlikeCommentMqDTO> unlikes = values.stream()
                .filter(op -> Objects.equals(LikeUnlikeCommentTypeEnum.UNLIKE.getCode(), op.getType()))
                .toList();
